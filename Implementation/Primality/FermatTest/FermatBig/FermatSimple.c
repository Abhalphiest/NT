 ...
 *
 * Usage:	FermatSimple <n> <b>, with <n> a positive integer to test for
 *		possible primality, and <b> the upper bound of the numbers to test mod n
...
unsigned long fermatTest(unsigned long n, unsigned long b);
...
int main(int argc, char** argv)
{
 ...
 unsigned long b = (unsigned long) strtol(argv[2], NULL, 10);
 if(b <= 0)
  usageExit("Your b is not a valid positive integer.");
 
 unsigned long result = fermatTest(n, b);
 if(result == 0)
 {
...
unsigned long fermatTest(unsigned long n, unsigned long b)
{
 ...
 while(a < b)	//havent hit the upper bound yet
 {
